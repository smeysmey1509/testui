(self["webpackChunkstorybook"] = self["webpackChunkstorybook"] || []).push([["vendors-node_modules_storybook_addon-essentials_dist_actions_preview_mjs-node_modules_storybo-517179"],{
...
/***/ "./node_modules/@storybook/web-components/dist/chunk-GKNNPQCW.mjs":
/*!************************************************************************!*\
  !*** ./node_modules/@storybook/web-components/dist/chunk-GKNNPQCW.mjs ***!
  \************************************************************************/
...
var{window:globalWindow}=_storybook_global__WEBPACK_IMPORTED_MODULE_0__.global;globalWindow.STORYBOOK_ENV="web-components";function isValidComponent(tagName){if(!tagName)return !1;if(typeof tagName=="string")return !0;throw new Error('Provided component needs to be a string. e.g. component: "my-element"')}function isValidMetaData(customElements){if(!customElements)return !1;if(customElements.tags&&Array.isArray(customElements.tags)||customElements.modules&&Array.isArray(customElements.modules))return !0;throw new Error(`You need to setup valid meta data in your config.js via setCustomElements().
    See the readme of addon-docs for web components for more details.`)}function setCustomElements(customElements){_storybook_global__WEBPACK_IMPORTED_MODULE_0__.global.__STORYBOOK_CUSTOM_ELEMENTS__=customElements;}function setCustomElementsManifest(customElements){_storybook_global__WEBPACK_IMPORTED_MODULE_0__.global.__STORYBOOK_CUSTOM_ELEMENTS_MANIFEST__=customElements;}function getCustomElements(){return _storybook_global__WEBPACK_IMPORTED_MODULE_0__.global.__STORYBOOK_CUSTOM_ELEMENTS__||_storybook_global__WEBPACK_IMPORTED_MODULE_0__.global.__STORYBOOK_CUSTOM_ELEMENTS_MANIFEST__}var{window,EventSource}=_storybook_global__WEBPACK_IMPORTED_MODULE_0__.global; true&&module.hot?.decline&&(module.hot.decline(),new EventSource("__webpack_hmr").addEventListener("message",function(event){try{let{action}=JSON.parse(event.data);action==="built"&&window.location.reload();}catch{}}));
...
/* harmony export */   parameters: () => (/* binding */ parameters)
/* harmony export */ });
/* harmony import */ var _chunk_GKNNPQCW_mjs__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./chunk-GKNNPQCW.mjs */ "./node_modules/@storybook/web-components/dist/chunk-GKNNPQCW.mjs");
/* harmony import */ var storybook_internal_docs_tools__WEBPACK_IMPORTED_MODULE_5__ = __webpack_require__(/*! storybook/internal/docs-tools */ "./node_modules/@storybook/core/dist/docs-tools/index.js");
...
function mapItem(item,category){let type;switch(category){case"attributes":case"properties":type={name:item.type?.text||item.type};break;case"slots":type={name:"string"};break;default:type={name:"void"};break}return {name:item.name,required:!1,description:item.description,type,table:{category,type:{summary:item.type?.text||item.type},defaultValue:{summary:item.default!==void 0?item.default:item.defaultValue}}}}function mapEvent(item){let name=item.name.replace(/(-|_|:|\.|\s)+(.)?/g,(_match,_separator,chr)=>chr?chr.toUpperCase():"").replace(/^([A-Z])/,match=>match.toLowerCase());return name=`on${name.charAt(0).toUpperCase()+name.substr(1)}`,[{name,action:{name:item.name},table:{disable:!0}},mapItem(item,"events")]}function mapData(data,category){return data&&data.filter(item=>item&&item.name).reduce((acc,item)=>{if(item.kind==="method")return acc;switch(category){case"events":mapEvent(item).forEach(argType=>{(0,tiny_invariant__WEBPACK_IMPORTED_MODULE_4__["default"])(argType.name,`${argType} should have a name property.`),acc[argType.name]=argType;});break;default:acc[item.name]=mapItem(item,category);break}return acc},{})}var getMetaDataExperimental=(tagName,customElements)=>{if(!(0,_chunk_GKNNPQCW_mjs__WEBPACK_IMPORTED_MODULE_0__.isValidComponent)(tagName)||!(0,_chunk_GKNNPQCW_mjs__WEBPACK_IMPORTED_MODULE_0__.isValidMetaData)(customElements))return null;let metaData=customElements.tags.find(tag=>tag.name.toUpperCase()===tagName.toUpperCase());return metaData||storybook_internal_client_logger__WEBPACK_IMPORTED_MODULE_1__.logger.warn(`Component not found in custom-elements.json: ${tagName}`),metaData},getMetaDataV1=(tagName,customElements)=>{if(!(0,_chunk_GKNNPQCW_mjs__WEBPACK_IMPORTED_MODULE_0__.isValidComponent)(tagName)||!(0,_chunk_GKNNPQCW_mjs__WEBPACK_IMPORTED_MODULE_0__.isValidMetaData)(customElements))return null;let metadata;return customElements?.modules?.forEach(_module=>{_module?.declarations?.forEach(declaration=>{declaration.tagName===tagName&&(metadata=declaration);});}),metadata||storybook_internal_client_logger__WEBPACK_IMPORTED_MODULE_1__.logger.warn(`Component not found in custom-elements.json: ${tagName}`),metadata},getMetaData=(tagName,manifest)=>manifest?.version==="experimental"?getMetaDataExperimental(tagName,manifest):getMetaDataV1(tagName,manifest),extractArgTypesFromElements=(tagName,customElements)=>{let metaData=getMetaData(tagName,customElements);return metaData&&{...mapData(metaData.members??[],"properties"),...mapData(metaData.properties??[],"properties"),...mapData(metaData.attributes??[],"attributes"),...mapData(metaData.events??[],"events"),...mapData(metaData.slots??[],"slots"),...mapData(metaData.cssProperties??[],"css custom properties"),...mapData(metaData.cssParts??[],"css shadow parts")}},extractArgTypes=tagName=>{let cem=(0,_chunk_GKNNPQCW_mjs__WEBPACK_IMPORTED_MODULE_0__.getCustomElements)();return extractArgTypesFromElements(tagName,cem)},extractComponentDescription=tagName=>{let metaData=getMetaData(tagName,(0,_chunk_GKNNPQCW_mjs__WEBPACK_IMPORTED_MODULE_0__.getCustomElements)());return metaData&&metaData.description};var LIT_EXPRESSION_COMMENTS=/<!--\?lit\$[0-9]+\$-->|<!--\??-->/g;function skipSourceRender(context){let sourceParams=context?.parameters.docs?.source,isArgsStory=context?.parameters.__isArgsStory;return sourceParams?.type===storybook_internal_docs_tools__WEBPACK_IMPORTED_MODULE_5__.SourceType.DYNAMIC?!1:!isArgsStory||sourceParams?.code||sourceParams?.type===storybook_internal_docs_tools__WEBPACK_IMPORTED_MODULE_5__.SourceType.CODE}function sourceDecorator(storyFn,context){let story=storyFn(),renderedForSource=context?.parameters.docs?.source?.excludeDecorators?context.originalStoryFn(context.args,context):story,source;if((0,storybook_internal_preview_api__WEBPACK_IMPORTED_MODULE_3__.useEffect)(()=>{let{id,unmappedArgs}=context;source&&storybook_internal_preview_api__WEBPACK_IMPORTED_MODULE_3__.addons.getChannel().emit(storybook_internal_docs_tools__WEBPACK_IMPORTED_MODULE_5__.SNIPPET_RENDERED,{id,source,args:unmappedArgs});}),!skipSourceRender(context)){let container=window.document.createElement("div");renderedForSource instanceof DocumentFragment?(0,lit__WEBPACK_IMPORTED_MODULE_2__.render)(renderedForSource.cloneNode(!0),container):(0,lit__WEBPACK_IMPORTED_MODULE_2__.render)(renderedForSource,container),source=container.innerHTML.replace(LIT_EXPRESSION_COMMENTS,"");}return story}var decorators=[sourceDecorator],parameters={docs:{extractArgTypes,extractComponentDescription,story:{inline:!0},source:{type:storybook_internal_docs_tools__WEBPACK_IMPORTED_MODULE_5__.SourceType.DYNAMIC,language:"html"}}},argTypesEnhancers=[storybook_internal_docs_tools__WEBPACK_IMPORTED_MODULE_5__.enhanceArgTypes];
